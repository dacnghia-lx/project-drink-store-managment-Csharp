    ...
    public partial class UC_ADMIN_STAFF : UserControl
    {
        ...
        private void DgNhanVien_SelectionChanged(object sender, EventArgs e)
        {
            if (dg_nhanvien.SelectedRows.Count > 0)
            {
                ...
                try
                {
                    var bytes = staff.GetImageByStaffId(maNV);
                    _anhBytes = bytes;
        ...
        private void btnChooseImage_Click(object sender, EventArgs e)
        {
            using (var ofd = new OpenFileDialog())
            {
                ...
                if (ofd.ShowDialog() == DialogResult.OK)
                {
                    ...
                    {
                        // Preview
                        ImgEmployee.Image = Image.FromFile(ofd.FileName);
                        // Read bytes
        ...
        private void updateStaff()
        {
            ...
            if (staff.UpdateStaff(MaNV, TenNV, DienThoai, GioiTinh, ChucVu, DiaChi, NgaySinh, NgayVaoLam, _anhBytes, updateImage))
            {
                resetDataTable();
        ...
        {
            getData();
            if (staff.AddStaff(MaNV, TenNV, DienThoai, GioiTinh, ChucVu, DiaChi, NgaySinh, NgayVaoLam, _anhBytes))
            {
                resetDataTable();
        ...
        {
            bool isEmpty = true;
            if (textBoxName.Text == "" || textboxPhone.Text == "" || comboBoxGender.Text == "" || comboBoxPosition.Text == "" || textBoxAddress.Text == "")
                isEmpty = true;
            else
                isEmpty = false;
            return isEmpty;
        ...
        private void clearData()
        {
            ...
            dateTimePickerDob.Text = "01/01/2025";
            comboBoxPosition.Text = "";
            textBoxAddress.Text = "";
            dateTimePickerStartDate.Text = "01/01/2025";
            // clear image preview and bytes
            ImgEmployee.Image = null;
            _anhBytes = null;
        }
